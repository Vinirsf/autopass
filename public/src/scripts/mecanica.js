...
const listContainer = document.getElementById('lista-mecanica');
...
useLocationBtn.addEventListener('click', () => {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            fetchEstabelecimentos(latitude, longitude, 'mecanica');
        });
    } else {
...
async function fetchEstabelecimentos(lat, lng, tipo) {
    ...
    data.forEach(estabelecimento => {
      ...
      <a href="/mecanica-detalhe.html?id=${estabelecimento.id}">Ver detalhes</a>
    `;
