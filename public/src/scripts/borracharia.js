...
const listContainer = document.getElementById('lista-borracharia');
...
useLocationBtn.addEventListener('click', () => {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            fetchEstabelecimentos(latitude, longitude, 'borracharia');
        });
    } else {
...
async function fetchEstabelecimentos(lat, lng, tipo) {
    ...
    data.forEach(estabelecimento => {
      ...
      <a href="/borracharia-detalhe.html?id=${estabelecimento.id}">Ver detalhes</a>
    `;
